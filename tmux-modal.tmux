...
#
# This binds `Shift-Up`, `Ctrl-n` and `Alt-p`, to `KBD_WIN_RESIZE_UP` (window
# resize up), `KBD_GOTO_WIN_NEXT` (go to next window) and `KBD_GOTO_SESS_PREV`
# (go to previous session), respectively.
#
...
bind-key -T $KT_CMD $KBD_CMD set-option key-table root
...
bind-key -T $KT_WIN_PANE $KBD_CMD set-option key-table root
EOF
...
bind-key -T $KT_WIN_RESIZE $KBD_CMD set-option key-table root
EOF
